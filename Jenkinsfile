        ...
        stage('Deploy to Remote Azure VM') {
            steps {
                ...
                withCredentials([
                    ...
                    script {
                        sh """
                        ssh -i \$SSH_KEY_VM1 \$AZURE_VM_USERNAME@xxx.xxx.xxx.xxx 'docker stop my-spring-app || true'
                        ssh -i \$SSH_KEY_VM1 \$AZURE_VM_USERNAME@xxx.xxx.xxx.xxx 'docker rm my-spring-app || true'
                        docker save my-spring-app | gzip | \
                            ssh -i \$SSH_KEY_VM1 \$AZURE_VM_USERNAME@xxx.xxx.xxx.xxx 'gunzip | docker load'
                        ssh -i \$SSH_KEY_VM1 \$AZURE_VM_USERNAME@xxx.xxx.xxx.xxx 'docker run -d -p 8081:8081 --name my-spring-app my-spring-app'
                        """
                    }
                }
            }
